...
export class TTLCache<T> {
  ...
  set(params: {
    ...
    const now = Date.now();
    const expiresAt = Number.isFinite(effectiveTtl) ? now + effectiveTtl : Infinity;
    const staleAt =
      Number.isFinite(expiresAt) && this.config.staleWhileRevalidate
  ...
  private evictOldest(): void {
    // Prefer evicting entries with finite expiry (e.g. registry resolutions); avoid evicting permanent (bootstrap) entries.
    let keyToEvict: string | undefined;
    let minExpiresAt = Infinity;
    for (const [key, entry] of this.cache) {
      if (Number.isFinite(entry.expiresAt) && entry.expiresAt < minExpiresAt) {
