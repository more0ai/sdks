 ...
 *
 * Resolution priority:
 * 1. Bootstrap (for system capabilities)
 * 2. Cache (for previously resolved capabilities)
 * 3. Registry (remote via NATS)
 */
...
export class ResolutionClient {
  ...
  private bootstrap: ResolvedBootstrap | undefined;
  private log: any;
  ...
  constructor(params: {
    ...
    this.bootstrap = params.deps.bootstrap;
  ...
  async resolve(input: ResolveInput): Promise<ResolveOutput> {
    if (this.bootstrap) {
      const bootstrapEntry = this.bootstrap.capabilities.get(input.cap);
      if (bootstrapEntry) {
        this.log.debug?.({ cap: input.cap }, `${SERVICE_NAME}:resolve - Using bootstrap`);
        return this.bootstrapToResolveOutput(bootstrapEntry, input.cap);
      }
    }
  ...
  private bootstrapToResolveOutput(entry: ResolvedBootstrapEntry, cap: string): ResolveOutput {
    const defaultNatsUrl = this.deps.defaultNatsUrl ?? "nats://127.0.0.1:4222";
    return {
      canonicalIdentity: `cap:@main/${cap}@${entry.version}`,
      natsUrl: entry.natsUrl ?? defaultNatsUrl,
      ...
      etag: entry.etag,
      methods: entry.methods.map((name) => ({
        name,
